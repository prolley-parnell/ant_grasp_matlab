...
%qualityObject = graspQuality();
%qualityObject = repmat(graspQuality(), [1, nExperiment]);
%nQuality = length(properties(graspQuality));
graspQarray = repmat(struct('force', [], 'friction', [], 'volume',[], 'epsilon', []), [nExperiment,1]);
...
parfor (n = 1:nExperiment, opts)
    ...
    qualityObject = gE.evaluateGoal(goalObj, env);
    graspQarray(n).force = RUNTIME_ARGS_i(n).GRASP.FORCE;
    graspQarray(n).friction = RUNTIME_ARGS_i(n).GRASP.OBJ_FRICTION;
    graspQarray(n).epsilon = qualityObject.epsilon;
    graspQarray(n).volume = qualityObject.volume;
...
cd(scriptFolder)
save('forceResults.mat', 'graspQarray')